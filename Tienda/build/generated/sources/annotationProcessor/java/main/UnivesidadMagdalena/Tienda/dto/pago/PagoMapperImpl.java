...
public class PagoMapperImpl implements PagoMapper {
    ...
    @Override
    public Pago pagoToSaveDtoToPago(PagoToSaveDto pagoToSaveDto) {
        ...
        pago.id( pagoToSaveDto.id() );
        pago.totalPago( pagoToSaveDto.totalPago() );
        pago.fechaPago( pagoToSaveDto.fechaPago() );
        pago.metodoPago( pagoToSaveDto.metodoPago() );

        return pago.build();
    }

    @Override
    public List<PagoDto> pagosToPagosDto(List<Pago> pagoList) {
        if ( pagoList == null ) {
            return null;
        }

        List<PagoDto> list = new ArrayList<PagoDto>( pagoList.size() );
        for ( Pago pago : pagoList ) {
            list.add( pagoToPagoDto( pago ) );
        }
    ...
    @Override
    public List<Pago> pagosDtoToPagos(List<PagoDto> pagoDtoList) {
        if ( pagoDtoList == null ) {
            return null;
        }

        List<Pago> list = new ArrayList<Pago>( pagoDtoList.size() );
        for ( PagoDto pagoDto : pagoDtoList ) {
            list.add( pagoDtoToPago( pagoDto ) );
        }
